  ...
  *buf='\0'; 
  ...
  if( !IS_NPC( ch ) )
    ...
    if( which_god( ch )==GOD_POLICE  && ch->level<98 )
      {
      if( ch->sex == 2 )
        strcpy( get_name_buffer, "Realm Protector " );
      else
        strcpy( get_name_buffer, "Realm Protector " );
      strcpy( buf, ch->name );
      *buf = UPPER( *buf );
    ...
    /* Order's Title - ORDER 7-24-99 */
    if( ch->pcdata->pvnum == 107 && !strcasecmp(ch->name,"order"))
      {
      strcpy( get_name_buffer, "The God of Reason " );
      strcpy( buf, ch->name );
      *buf = UPPER( *buf );
      strcat( get_name_buffer, buf);
      return( get_name_buffer );
     ...
     /* Order's Title - ORDER 7-24-99 */
    if( ch->pcdata->pvnum == 71183 && !strcasecmp(ch->name,"grog"))
      {
      strcpy( get_name_buffer, "Grand Knight " );
      strcpy( buf, ch->name );
      *buf = UPPER( *buf );
      strcat( get_name_buffer, buf);
      return( get_name_buffer );
      }
   /* emps's Title - ORDER 7-24-99 */
    if( ch->pcdata->pvnum == 31793 && !strcasecmp(ch->name,"emperor"))
      {
      strcpy( get_name_buffer, "High Wizard " );
      strcpy( buf, ch->name );
      *buf = UPPER( *buf );
    ...
    if( ch->level > 96 )
      {
      if( ch->sex == 2 )
        strcpy( get_name_buffer, "Grand Countess " );
      else
        strcpy( get_name_buffer, "High Wizard " );
      strcpy( buf, ch->name );
      *buf = UPPER( *buf );
